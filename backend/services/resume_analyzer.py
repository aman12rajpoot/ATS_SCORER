...
def analyze_full_resume(
    ...
    # Parse resume and job description in parallel
    parsed_resume = None
    parsed_jd = None
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        parsed_resume = resume_future.result()
        
        # Wait for job description parsing if submitted
        if jd_future:
            try:
                parsed_jd = jd_future.result()
            except Exception as e:
                logger.warning(f"Job description parsing failed: {e}")
                parsed_jd = None
