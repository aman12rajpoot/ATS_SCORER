...
def _extract_pdf_with_pdfplumber(file_data: bytes) -> str:
    ...
    try:
        with pdfplumber.open(io.BytesIO(file_data)) as pdf:
            # Limit to first 50 pages to prevent hanging on huge files
            for page in pdf.pages[:50]:
                try:
                    ...
                    log_warning(f'Failed to extract page text: {e}', context='_extract_pdf_with_pdfplumber')
                    continue
        ...
        log_error(f'pdfplumber error: {e}', context='_extract_pdf_with_pdfplumber')
        raise
    ...
    try:
        ...
        for i in range(pages_to_process):
            try:
                ...
                log_warning(f'Failed to extract page {i}: {e}', context='_extract_pdf_with_pypdf2')
                continue
        ...
        log_error(f'PyPDF2 error: {e}', context='_extract_pdf_with_pypdf2')
        raise
...
def extract_text_from_pdf(file_data: bytes) -> str:
    ...
    try:
        ...
        log_warning(f'pdfplumber extraction failed: {e}', context='extract_text_from_pdf')
    
    # Fall back to PyPDF2
    try:
        return _extract_pdf_with_pypdf2(file_data)
    except Exception as e:
        log_warning(f'PyPDF2 extraction also failed: {e}', context='extract_text_from_pdf')
    
    # Both failed
    log_error('Both PDF extraction methods failed', context='extract_text_from_pdf')
    raise FileParsingError(
        ...
        'Please ensure it contains selectable text, or try converting it using Adobe or another PDF tool.'
    )
